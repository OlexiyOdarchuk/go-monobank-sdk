...
3. **Локальні перевірки** (фейл — стоп):
   ...
   python3 scripts/check-spec-drift.py    # чи не змінилися доки monobank
   ```

   Останній пункт — не формальність: SDK написаний за опублікованими
   специфікаціями, і якщо котрась із них зрушила, код міг тихо
   розійтися з API. При спрацюванні звірити відповідний пакет і
   оновити піни через `--update` у тому ж коміті.
