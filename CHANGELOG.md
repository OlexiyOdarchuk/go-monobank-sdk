...
- **Новий пакет `openbanking` — monobank Open Banking (PSD2 /
  Berlin Group), 17 операцій.** Згоди на доступ до рахунків
  (`/v2/consents/account-access`), інформація про рахунки
  (`/v2/accounts`, баланси, транзакції), ініціювання платежів
  (`/v2/payments/{paymentProduct}`) з окремим флоу авторизації, плюс
  дві sandbox-операції видачі тестового сертифіката. Окремий хост
  `openbanking.mono.bank`, автентифікація **лише mTLS** (QWAC-сертифікат
  у TLS-рукостисканні) — токена немає; хелпери
  [NewMTLSHTTPClient] / [NewMTLSHTTPClientFromFiles] будують
  готовий `*http.Client` для [monobank.WithHTTPClient]. Пакет
  побудований на спільному ядрі [monobank.Client], тож ретраї,
  ліміти, хуки, OpenTelemetry і `errors.Is` із сентинелами працюють
  так само, як у решті SDK. DTO редагують персональні дані в логах
  (IBAN, ПІБ, РНОКПП/ЄДРПОУ, призначення платежу) за тими ж правилами,
  що й `bank`, включно з обгортками на зрізах і структурах-контейнерах;
  `MTLSConfig` не пускає в лог приватний ключ QWAC.

  Це **не** заміна пакета `business` (`corp-api.monobank.ua`,
  `/ext/v1`, один `X-Token`) — інший продукт, для third-party-
  провайдерів зі згодами клієнта.

- **`acquiring`: повернення за POS-транзакцією.** [Client.POSTransactionCancel]
  — `POST /api/merchant/pos-transaction-cancel`. Продаж на фізичному
  POS-терміналі не проходив через `CreateInvoice`, тож `CancelInvoice`
  до нього не дістається — повернення адресується за RRN оригінальної
  операції. Часткові повернення дозволені.
- **`acquiring`: статус T2P-платежу.** [Client.T2PPaymentStatus] —
  `GET /api/merchant/t2p/terminal/payment/external/status` за
  `externalPaymentId`. Доки прямо кажуть, що тіло відповіді ідентичне
  payload вебхука `callbackSuccess`/`callbackFail` — саме щоб
  інтегратор міг не піднімати власний сервер для вебхуків. Пошук
  обмежений останніми 90 днями.

  > Набір значень статусу в доках суперечливий: проза таблиці називає
  > `failed`, а приклад поруч — `rejected`. SDK оголошує всі варіанти
  > і зберігає невідомі як є; не покладайтесь на одну константу
  > невдачі. Так само `cardMask` у таблиці написаний як `Visa`, а в
  > прикладі — `visa`: порівнюйте без урахування регістру.
- **`installment`: QR-кошик.** [Client.CreateQRCart] і
  [Client.CancelQRCart] — `POST /api/v1/qr/cart` та
  `/api/v1/qr/cart/cancel`. Кошик чіпляється до QR-коду магазину,
  який клієнт сканує в застосунку. На відміну від `CreateOrder`,
  запит не містить ні телефону клієнта, ні переліку програм.
- **`bank.ClientInfo.ManagedClients`** — перелік клієнтів, які надали
  бухгалтеру доступ до своїх ФОП-рахунків. Події по цих рахунках банк
  НЕ надсилає, їх треба опитувати.
- **`business`: фактичний платник у бюджетних платежах.**
  [PaymentRequest.UltimateDebit] (`{id, name}`) — без нього бюджетний
  платіж не сформувати; для звичайних платежів банк поле ігнорує, тож
  воно вказівник і в payload не потрапляє. Симетрично
  [StatementItem.UltimateDebit] (`{name, idCode}`) — ці дані раніше
  мовчки губилися при розборі виписки. Різні імена полів на дроті
  (`id` проти `idCode`), тому це два типи, а не один.
- **`corporate.Document.HashType`** — вибір алгоритму гешування
  документа для monoКЕП: [HashGost] (ГОСТ 34.311-95, застосовується
  за замовчуванням) або [HashDstu256] (ДСТУ 7564 «Купина-256»).
  Раніше ДСТУ був недосяжний.

### Fixed

- **`business`: успішне видалення розрахункових листів поверталося як
  помилка.** [Client.DeletePayslips] і [Client.DeleteImport] чекали
  HTTP 200, а офіційна специфікація (`client-specification.yaml`
  v1.0.2) для обох операцій документує **204** і 200 не описує взагалі
  — тобто виклик, що фактично спрацював, віддавав `*APIError`. Тепер
  приймається 204 (і 200 теж — тіла там немає в будь-якому разі).
- **`business`: payroll-дані повністю потрапляли в лог.** Пакет не мав
  редагування взагалі, хоча несе найчутливіші дані SDK: ПІБ
  співробітників, ІНН, серії й номери документів, IBAN, PAN, суми
  зарплат і рядки розрахункових листів. `slog.Any` виводив усе це
  дослівно. Додано `LogValue` на 20+ типів разом із обгортками на
  зрізах і контейнерах; закріплено тестом на 25 випадків.

  Побічний ефект: [Client.Accounts] тепер повертає [Accounts], а
  [Client.Statement] — [StatementItems] замість голих зрізів (саме
  іменований тип і дає редагування). Присвоєння в обидва боки з
  `[]Account` / `[]StatementItem` лишається валідним, тож наявний код
  компілюється без змін.
- **`bank`: виписка й банки повністю потрапляли в лог.**
  `Transaction`/`Transactions` і `Jar`/`Jars` не мали `LogValue`, тож
  `slog.Any` виводив призначення платежу, IBAN і ЄДРПОУ контрагента,
  його ім'я та назви банок дослівно. Закрито тим самим набором
  редакторів, що й решта пакета.
- **`openbanking`: голий сегмент `.` або `..` в ідентифікаторі
  перебудовував запит.** `url.PathEscape` не чіпає крапку, а
  резолвінг відносно базового URL згортає dot-сегменти — тож
  `RevokeConsent(ctx, "..")` слав `DELETE /ext/v2/consents/` замість
  адресованого ресурсу. Такі значення тепер відхиляються з
  [ErrEmptyID] до відправлення. Ідентифікатори зі слешами
  екранувалися коректно і раніше.
- **`bank`: логування зрізу рахунків зливало IBAN у відкритий лог.**
  `slog` не «дотягується» крізь зріз до `LogValue` елемента, тож
  `slog.Any("accounts", info.Accounts)` друкував `Account` як сирі
  структури — повні IBAN-и і маски карток, попри наявний
  `Account.LogValue`. Додано `LogValue` на рівні колекцій
  ([Accounts] і новий [ManagedClients]); для `ManagedClients` це
  критичніше, бо там ще й ПІБ та РНОКПП третьої особи. Редагування
  через `ClientInfo` працювало коректно і раніше — текла лише пряма
  передача зрізу. Закріплено тестом `TestCollections_LogValue`.

### Deprecated

- **[Client.GuaranteeLetterData]** (`installment`) — офіційні доки
  позначили `/api/order/data/for/guarantee/letter` як застарілий.
  Використовуйте [Client.GuaranteeLetterDataV2].

### Changed

- **Додано інтеграційні тести для нових ендпоінтів.** Досі жоден із
  них не викликався по-справжньому — усе покриття було на `httptest`,
  тобто перевіряло, що SDK шле те, що ми ж і вирішили. Тепер є 18
  тестів під тегом `integration` для `acquiring`, `installment`,
  `business` і `openbanking`; кожен скіпається без своїх змінних, тож
  звичайний `go test ./...` не зачіпає. Виклики, що рухають гроші або
  видаляють дані, потребують окремого явного опт-іну понад креденшали.
  Найцінніші з них: перевірка mTLS-рукостискання без QWAC, звірка
  реальної відповіді T2P із нашою структурою і підтвердження 204 на
  видаленні розрахункових листів.
- **Додано щотижневу перевірку дрейфу специфікацій**
  (`scripts/check-spec-drift.py` + воркфлоу `Spec drift`). Усі пʼять
  опублікованих специфікацій monobank запінено за вмістом у
  `testdata/spec-checksums.json`; зміна будь-якої валить джоб. Саме
  непомічений дрейф і призвів до багу з 204 вище. Для ReDoc-сторінок
  хешується витягнутий OpenAPI-документ, а не HTML, щоб зміни обгортки
  не давали хибних спрацювань.
- Посилання на доки в `acquiring` переведено зі застарілого дзеркала
  `api.monobank.ua/docs/acquiring.html` (спек `v2410`, у ньому немає
  subscription/monopay/split/t2p) на актуальний
  `monobank.ua/api-docs/acquiring/*` — 18 посилань. Два
  (`employee/list`, `invoice/sync-payment`) свідомо лишено на старому
  дзеркалі: на новому сайті сторінок для них немає.
- **Мінімальний тулчейн: Go 1.25 → Go 1.26.** `golang.org/x/sync` v0.23.0
  вимагає `go 1.26.0`, тож обидва модулі (кореневий і `otelmonobank`),
  `go.work` і CI піднято на 1.26. Споживачам на Go 1.25 треба оновити
  тулчейн — версію `2.0.x` це не зачіпає. Матрицю CI звужено до `1.26.x`.
...
- **Виписка тепер добирає вікна понад 500 транзакцій.** Mono віддає
  ...
  ліміт описаний в офіційній специфікації `/personal/statement`. Раніше
  `TransactionsRange` / `TransactionsRangeIter` (`personal` і `corporate`)
  різали період лише на 31-денні вікна й тихо губили транзакції, якщо в
  одному вікні їх було понад 500. Тепер повне вікно добирається повторними
  запитами з переанкоруванням `to` на найстарішу секунду й дедупом за id —
  той самий safe-на-однакову-секунду підхід, що вже застосований у
